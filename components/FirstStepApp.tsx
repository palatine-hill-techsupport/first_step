...
export function DemoModeBanner() {
  return (
    <div className="demo-banner" role="note">
      ...
      <a href={deploymentPath("/demo")}>See demo roles <ArrowRight size={15} /></a>
    </div>
  );
}
  ...
  return (
    <section className={full ? "urgent-panel" : "urgent-strip"} aria-labelledby={full ? "urgent-heading" : undefined}>
      ...
      <div className="urgent-actions">
        ...
        <a className="button button-light" href={deploymentPath("/pathways/safe-tonight")}>See all urgent options</a>
      </div>
    </section>
  );
}
    ...
    <>
      <header className="app-header">
        ...
        <nav className="desktop-nav" aria-label="Main navigation">
          {navigation.map(([label, href]) => <a key={href} href={deploymentPath(href)} aria-current={isNavigationActive(pathname, href) ? "page" : undefined}>{label}</a>)}
        </nav>
...
const reportDemoMessage = "Demonstration pilot data. These sample figures show how first_step could report a sponsor-funded pilot. They are not real participant outcomes.";

function ReportDemoLabel({ compact = false }: { compact?: boolean }) {
  return <p className={`report-demo-label ${compact ? "compact" : ""}`}><AlertTriangle aria-hidden="true" />{compact ? <><strong>Demonstration data</strong><span className="sr-only">{reportDemoMessage}</span></> : <span>{reportDemoMessage}</span>}</p>;
}
...
function ReportMetricCard({ metric }: { metric: typeof demonstrationReport.coreMetrics[number] }) {
  ...
  return <article className="report-metric-card"><strong>{value}</strong><h3>{metric.label}</h3><p>{metric.note}</p>{metric.denominator && <small>{metric.denominator}</small>}<span>{metric.status}</span></article>;
}

const reportSectionLinks = [
  ["Overview", "overview"], ["Conversion", "conversion"], ["Channels", "channels"], ["Referrals", "referrals"],
  ["Outcomes", "outcomes"], ["Feedback", "feedback"], ["Costs", "costs"], ["Boundaries", "boundaries"], ["Next pilot", "next-pilot"],
] as const;
...
function ReportSectionNav() {
  ...
  useEffect(() => {
    const sections = reportSectionLinks.map(([, id]) => document.getElementById(id)).filter(Boolean) as HTMLElement[];
  ...
  return <nav className="report-section-nav" aria-label="Report sections"><div>{reportSectionLinks.map(([label, id]) => <a key={id} href={`#${id}`} aria-current={active === id ? "location" : undefined}>{label}</a>)}</div></nav>;
}

function ParticipantFunnel() {
  const stages = demonstrationReport.funnel;
  return <section id="conversion" className="report-section report-funnel-section" aria-labelledby="funnel-heading"><ReportDemoLabel compact /><div className="report-section-heading"><div><span className="eyebrow">Participant funnel</span><h2 id="funnel-heading">The next step became narrower at each stage.</h2></div><p>Counts show an opt-in journey, not a sales conversion target.</p></div><div className="funnel-list" role="list">{stages.map((stage, index) => { const previous = stages[index - 1]?.value ?? stage.value; const width = Math.max(18, Math.round((stage.value / stages[0].value) * 100)); return <article key={stage.id} className={index === 1 || index === 2 ? "funnel-dropoff" : ""} role="listitem"><div className="funnel-label"><span>{index + 1}</span><strong>{stage.label}</strong></div><div className="funnel-bar-wrap" aria-hidden="true"><div className="funnel-bar" style={{ width: `${width}%` }} /></div><strong className="funnel-count">{stage.value.toLocaleString("en-AU")}</strong><small>{index ? `${reportPercent(stage.value, previous)}% from the previous step` : "Starting point"}</small></article>; })}</div><details className="report-detail-disclosure"><summary>View accessible funnel table</summary><div className="report-table-wrap"><table><caption>Participant funnel counts and conversion from the previous stage</caption><thead><tr><th scope="col">Stage</th><th scope="col">Count</th><th scope="col">Conversion from previous</th></tr></thead><tbody>{stages.map((stage, index) => <tr key={stage.id}><th scope="row">{stage.label}</th><td>{stage.value.toLocaleString("en-AU")}</td><td>{index ? `${reportPercent(stage.value, stages[index - 1].value)}%` : "—"}</td></tr>)}</tbody></table></div></details><div className="funnel-annotation"><AlertTriangle aria-hidden="true" /><p><strong>Largest drop-offs are visible, not hidden.</strong> Most visitors browsed without starting a pathway. That is not automatically failure. Some people may have found the information they needed without creating an account or booking.</p></div></section>;
}

function ImpactDashboardPageLegacy() {
...
function ImpactDashboardPage() {
  const report = demonstrationReport;
  if (!report?.pilot) return <ImpactDashboardPageLegacy />;
  const { metrics } = report;
  const appointmentAttendance = reportPercent(metrics.appointmentsCompleted, metrics.appointmentsBooked);
  const referralConsent = reportPercent(metrics.referralsConsented, metrics.referralsOffered);
  const partnerAcceptance = reportPercent(metrics.referralsAccepted, metrics.referralsConsented);
  ...
  const costPerReferral = Math.round(metrics.pilotBudget / metrics.referralsAccepted);
  const costPerPlacement = Math.round(metrics.pilotBudget / metrics.placementsBegan);
  const referralStages = [["Offered", report.referrals.offered], ["Consented", report.referrals.consented], ["Sent", report.referrals.sent], ["Accepted", report.referrals.accepted]] as const;

  return <div className="impact-dashboard">
    <section className="report-header">
      <div>
        <span className="eyebrow light">{report.pilot.name} · Demonstration report</span>
        <h1>Measure the bridge.<br /><em>Protect the person.</em></h1>
        <p>{report.pilot.period}. One public activation, an online channel that stayed open, and consent-led next steps.</p>
      </div>
      <aside className="report-summary-panel" aria-label="Report summary">
        <p className="report-summary-kicker">Demonstration report</p>
        <dl className="report-summary-meta">
          <div><dt>Report</dt><dd>Pilot 001</dd></div>
          <div><dt>Period</dt><dd>Eight-week demonstration pilot</dd></div>
          <div><dt>Public activation</dt><dd>One public activation</dd></div>
          <div><dt>Online channel</dt><dd>Active throughout</dd></div>
          <div><dt>Last updated</dt><dd>{report.pilot.lastUpdated}</dd></div>
        </dl>
        <label><span>Reporting period</span><select aria-label="Reporting period" defaultValue="pilot-001"><option value="pilot-001">Pilot 001 · Eight weeks</option></select></label>
        <div className="report-header-actions"><button className="button button-mint" type="button" onClick={() => window.print()}><FileDown aria-hidden="true" /> Print report</button><a className="button button-outline" href={deploymentPath("/")}>Back to first_step</a></div>
      </aside>
    </section>
    <div className="report-content">
      <ReportDemoLabel />
      <ReportSectionNav />
      <section id="overview" className="report-executive report-section report-priority-section" aria-labelledby="executive-heading">
        <div><span className="eyebrow">Executive summary</span><h2 id="executive-heading">Did the bridge work?</h2><p>The demonstration suggests that first_step can turn low-pressure public and online contact into real conversations, referrals and paid opportunities. The strongest result is not reach alone. It is the number of people who chose a second step.</p><p className="report-reading-note"><strong>Read this first:</strong> the figures show a connected service journey, not proven long-term outcomes.</p></div>
        <div className="summary-findings"><article><UsersRound aria-hidden="true" /><h3>People used both doors</h3><p>The public pop-up generated visibility. The online channel gave people a quieter way to return later.</p></article><article><MessageCircle aria-hidden="true" /><h3>Human support mattered</h3><p>Appointments and warm referrals were the points where a browse became a chosen next step.</p></article><article><CircleHelp aria-hidden="true" /><h3>The bridge still loses people</h3><p>A meaningful number of users began a pathway but did not complete it. The next pilot should test shorter flows and stronger follow-up choices.</p></article></div>
      </section>
      <section id="outcomes" className="report-section" aria-labelledby="core-metrics-heading"><ReportDemoLabel compact /><div className="report-section-heading"><div><span className="eyebrow">Practical outcomes</span><h2 id="core-metrics-heading">Count the practical next step.</h2></div><p>One reporting period only. No year-on-year comparison is implied.</p></div><div className="report-metric-grid">{report.coreMetrics.map((metric) => <ReportMetricCard key={metric.id} metric={metric} />)}</div><p className="calculation-note">{pathwayCompletion}% completed a guided pathway; {appointmentAttendance}% of booked appointments were attended; {referralConsent}% of offered referrals were consented to; and {partnerAcceptance}% of consented referrals were accepted by a partner.</p></section>
      <ParticipantFunnel />
      <section id="channels" className="report-section channel-section" aria-labelledby="channels-heading"><ReportDemoLabel compact /><div className="report-section-heading"><div><span className="eyebrow">Entry channels</span><h2 id="channels-heading">Two doors, one connected service.</h2></div><p>Accepted referrals were evenly split across the two entry points. The channels appear to support one another rather than compete.</p></div><div className="channel-comparison">{report.channels.map((channel) => <article key={channel.name}><span className="comparison-kicker">{channel.interactions ? "In-person entry" : "Digital entry"}</span><h3>{channel.name}</h3><p>{channel.note}</p><dl>{channel.interactions ? <><div><dt>Interactions</dt><dd>{channel.interactions}</dd></div><div><dt>Attributed platform visits</dt><dd>{channel.visits}</dd></div></> : <div><dt>Platform visits</dt><dd>{channel.visits.toLocaleString("en-AU")}</dd></div>}<div><dt>Sign-ups</dt><dd>{channel.signUps}</dd></div><div><dt>Appointments booked</dt><dd>{channel.appointments}</dd></div><div><dt>Accepted referrals</dt><dd>{channel.acceptedReferrals}</dd></div></dl></article>)}</div><details className="report-detail-disclosure"><summary>View detailed comparison</summary><div className="report-table-wrap"><table><caption>Entry-channel totals in this demonstration report</caption><thead><tr><th scope="col">Channel</th><th scope="col">Visits</th><th scope="col">Sign-ups</th><th scope="col">Booked</th><th scope="col">Accepted referrals</th></tr></thead><tbody>{report.channels.map((channel) => <tr key={channel.name}><th scope="row">{channel.name}</th><td>{channel.visits}</td><td>{channel.signUps}</td><td>{channel.appointments}</td><td>{channel.acceptedReferrals}</td></tr>)}</tbody></table></div></details></section>
      <section id="referrals" className="report-section referral-section" aria-labelledby="referrals-heading"><ReportDemoLabel compact /><div className="report-section-heading"><div><span className="eyebrow">Referral outcomes</span><h2 id="referrals-heading">A link is not a warm referral.</h2></div><p>Accepted means the delivery partner accepted or began the referral process. It does not mean the participant’s underlying issue was resolved.</p></div><div className="referral-sequence">{referralStages.map(([label, value], index) => <article key={label} className={index === referralStages.length - 1 ? "final-stage" : ""}><strong>{value}</strong><span>{label}</span>{index > 0 && <small>{reportPercent(value, referralStages[index - 1][1])}% from previous</small>}{index < referralStages.length - 1 && <ArrowRight aria-hidden="true" />}</article>)}</div><div className="referral-layout"><div><h3>Accepted referral categories</h3><ul className="category-bars">{report.referrals.categories.map(([label, value]) => <li key={label}><span>{label}</span><i style={{ width: `${(value / report.referrals.accepted) * 100}%` }}><b>{value}</b></i></li>)}</ul></div><aside><h3>Accepted means process started</h3><p className="referral-definition">Accepted means the delivery partner accepted or began the referral process. It does not mean the participant’s underlying issue was resolved.</p><h3>Other referral outcomes</h3><dl><div><dt>Awaiting response</dt><dd>{report.referrals.awaiting}</dd></div><div><dt>Participant declines</dt><dd>{report.referrals.declined}</dd></div><div><dt>Not sent after reconsideration</dt><dd>{report.referrals.reconsidered}</dd></div></dl></aside></div></section>
      <section id="outcomes-detail" className="report-section report-two-column" aria-label="Confidence and paid work outcomes"><article className="confidence-card"><ReportDemoLabel compact /><span className="eyebrow">Confidence and readiness</span><h2>Confidence to take the next step</h2><p>Participant self-rating. It is not a clinical measure. Only matched before-and-after responses are included.</p><div className="rating-steps"><div><strong>{report.confidence.before}</strong><span>Starting rating / 5</span><div className="confidence-track"><i style={{ width: `${report.confidence.before / 5 * 100}%` }} /></div></div><ArrowRight aria-hidden="true" /><div><strong>{report.confidence.after}</strong><span>Follow-up rating / 5</span><div className="confidence-track"><i style={{ width: `${report.confidence.after / 5 * 100}%` }} /></div></div></div><p className="calculation-note">Average change: +{(report.confidence.after - report.confidence.before).toFixed(1)} across {report.confidence.responses} matched responses. The sample is small.</p></article><article className="placement-card"><ReportDemoLabel compact /><span className="eyebrow">Paid-work outcomes</span><h2>Work counts only when it is paid or formally structured.</h2><div className="placement-stats"><div><strong>{report.placements.created}</strong><span>Paid opportunities created</span></div><div><strong>{report.placements.began}</strong><span>Participants began a placement</span></div><div><strong>{report.placements.anotherPathway}</strong><span>Chose another pathway</span></div><div><strong>{report.placements.unmatched}</strong><span>Opportunity remained unmatched</span></div></div><p className="boundary-note"><BriefcaseBusiness aria-hidden="true" /> General expressions of interest are not counted as paid opportunities. Sustained employment is not implied.</p></article></section>
      <section id="feedback" className="report-section feedback-section" aria-labelledby="feedback-heading"><ReportDemoLabel compact /><div className="report-section-heading"><div><span className="eyebrow">Participant feedback</span><h2 id="feedback-heading">Sample feedback, not borrowed stories.</h2></div><p>Sample participant feedback written for this demonstration. These are not real participant quotations.</p></div><div className="quote-grid">{report.participantFeedback.quotes.map((quote, index) => <blockquote key={quote}><span className="quote-mark" aria-hidden="true">“</span><small>Demonstration feedback · 0{index + 1}</small><p>“{quote}”</p></blockquote>)}</div><div className="feedback-stat-grid"><div><strong>{report.participantFeedback.rating} / 5</strong><span>Experience rating</span></div><div><strong>{report.participantFeedback.respected}%</strong><span>Felt respected</span></div><div><strong>{report.participantFeedback.understoodNextStep}%</strong><span>Understood the next step</span></div><div><strong>{report.participantFeedback.wouldReturn}%</strong><span>Would return</span></div></div><p className="calculation-note">Based on {report.participantFeedback.responses} demonstration responses.</p></section>
      <section className="report-section stakeholder-section" aria-labelledby="stakeholder-heading"><ReportDemoLabel compact /><div className="report-section-heading"><div><span className="eyebrow">Partner and sponsor feedback</span><h2 id="stakeholder-heading">Useful scrutiny is part of the pilot.</h2></div><p>Small samples lead with counts; percentages are secondary and never stand alone.</p></div><div className="stakeholder-grid"><article><HeartHandshake aria-hidden="true" /><h3>Partner feedback</h3><p className="sample-base">Based on 5 partner respondents.</p><dl><div><dt>Interested in another pilot</dt><dd>4 of 5 <small>(80%)</small></dd></div><div><dt>Enough referral information</dt><dd>83% <small>(n=5)</small></dd></div><div><dt>Referral boundaries clear</dt><dd>92% <small>(n=5)</small></dd></div></dl></article><article><Banknote aria-hidden="true" /><h3>Sponsor feedback</h3><p className="sample-base">Based on 4 sponsor respondents.</p><dl><div><dt>Indicated renewal interest</dt><dd>3 of 4 <small>(75%)</small></dd></div><div><dt>Reporting felt appropriately cautious</dt><dd>4 of 4 <small>(100%)</small></dd></div><div><dt>Wanted longer-term reporting</dt><dd>2 of 4 <small>(50%)</small></dd></div></dl></article></div></section>
      <section id="costs" className="report-section cost-section" aria-labelledby="cost-heading"><ReportDemoLabel compact /><div><span className="eyebrow">Cost and value</span><h2 id="cost-heading">Put the whole pilot cost beside the selected outcomes.</h2></div><div className="cost-grid"><div><strong>{reportCurrency(metrics.pilotBudget)}</strong><span>Full demonstration pilot cost</span></div><div><strong>{reportCurrency(costPerBoothInteraction)}</strong><span>Per booth interaction</span></div><div><strong>{reportCurrency(costPerAppointment)}</strong><span>Per completed appointment</span></div><div><strong>{reportCurrency(costPerReferral)}</strong><span>Per accepted referral</span></div><div><strong>{reportCurrency(costPerPlacement)}</strong><span>Per participant beginning a paid placement</span></div></div><p className="cost-caveat">These figures divide the full pilot cost by selected outcomes. They are not direct unit costs and should not be read in isolation.</p><p className="calculation-note">The pilot budget also funds youth-worker availability, food and transport support, platform access, safeguarding, booth delivery, referral coordination and evaluation. Referrals and placements were not purchased.</p></section>
      <section className="report-section learnings-section"><div className="report-section-heading"><div><span className="eyebrow">What the pilot learned</span><h2>Keep the useful parts. Test the rest.</h2></div></div><div className="learning-grid">{report.learnings.map(([title, copy], index) => <article key={title}><span>0{index + 1}</span><h3>{title}</h3><p>{copy}</p></article>)}</div></section>
      <section id="limitations" className="report-section limitations-section" aria-labelledby="limitations-heading"><AlertTriangle aria-hidden="true" /><div><span className="eyebrow">Credibility boundary</span><h2 id="limitations-heading">What this pilot cannot tell us yet</h2><p>These omissions are not a weakness in the report. They are the questions a careful next pilot must answer.</p><ul>{report.limitations.map((item) => <li key={item}><Check aria-hidden="true" /> {item}</li>)}</ul></div></section>
      <section id="boundaries" className="privacy-report-boundary" aria-labelledby="privacy-heading"><ShieldCheck aria-hidden="true" /><div><span className="eyebrow">Privacy and sponsor boundaries</span><h2 id="privacy-heading">Measure the bridge. Protect the person.</h2><p><strong>Sponsor funding does not purchase access to participant data.</strong></p><div className="boundary-columns"><section><h3>Sponsors and public partners may see</h3><ul>{["Aggregate engagement totals", "Grouped pathway interests", "Conversion rates", "Anonymised feedback", "Overall referral outcomes", "Broad financial reporting"].map((item) => <li key={item}><Check aria-hidden="true" /> {item}</li>)}</ul></section><section><h3>Sponsors and public partners do not see</h3><ul>{["Names", "Contact details", "Appointment messages", "Individual referral records", "Personal histories", "Worker notes", "Identifiable journeys", "Participant-level dashboards"].map((item) => <li key={item}><X aria-hidden="true" /> {item}</li>)}</ul></section></div></div></section>
      <section id="next-pilot" className="report-section recommendations-section" aria-labelledby="recommendations-heading"><div><span className="eyebrow">Next-pilot recommendations</span><h2 id="recommendations-heading">Scale only what proves useful, safe and worth returning to.</h2></div><ol>{report.recommendations.map((item, index) => <li key={item}><span>{index + 1}</span><p>{item}</p></li>)}</ol></section>
      <p className="report-print-footer">first_step · Demonstration report · Pilot 001 · Last updated {report.pilot.lastUpdated} · Aggregate reporting only</p>
    </div>
  </div>;
}
...
function DashboardNav({ role }: { role: "participant" | "worker" | "admin" | "committee" }) {
  const pathname = usePathname() || "/";
  const items = role === "participant" ? [["Overview", "/account"], ["Appointments", "/account/appointments"], ["Saved steps", "/account/pathways"], ["Referrals", "/account/referrals"]] : role === "worker" ? [["Today", "/worker"], ["Appointments", "/worker/appointments"], ["Availability", "/worker/availability"], ["Referrals", "/worker/referrals"]] : role === "admin" ? [["Pilot overview", "/admin"], ["Workers", "/admin/workers"], ["Resources", "/admin/resources"], ["Partners", "/admin/partners"], ["Locations", "/admin/locations"], ["Service settings", "/admin/service-settings"], ["Impact", "/admin/impact"]] : [["Committee overview", "/committee"], ["Impact dashboard", "/impact-dashboard"]];
  return <aside className={`dashboard-nav dashboard-nav-${role}`}><BrandLogo /><span>{role === "participant" ? "Jamie’s account" : role === "worker" ? "Worker space · Maya" : role === "admin" ? "Pilot administration" : "Governance committee"}</span><nav>{items.map(([label, href]) => <a key={href} href={href} aria-current={pathname === href ? "page" : undefined}>{label}<ChevronRight /></a>)}</nav><a href="/demo" className="switch-role">Switch demo role</a></aside>;
}
...
function CommitteePage() {
  const committeeQuestions = [
    { number: "01", title: "Did people choose a second step?", copy: "Yes, but the journey narrows quickly after the first interaction. The committee should ask what makes returning easier.", status: "Evidence: promising", icon: <BarChart3 aria-hidden="true" /> },
    { number: "02", title: "Is the service safe to widen?", copy: "The operating boundaries are clear: consent-led referrals, qualified workers and aggregate reporting. Safeguarding still needs to be checked at each new site.", status: "Decision: conditional", icon: <ShieldCheck aria-hidden="true" /> },
    { number: "03", title: "What must be tested next?", copy: "Longer follow-up, stronger return-later options and more housing and paid-work partners matter more than simply increasing reach.", status: "Action: commission learning", icon: <Sparkles aria-hidden="true" /> },
  ];
  return <div className="dashboard-page committee-dashboard"><DashboardNav role="committee" /><main><div className="dashboard-welcome committee-welcome"><div><span className="eyebrow">Committee demo · aggregate view</span><h1>Make the next call with the whole picture.</h1><p>Review what the pilot suggests, what it cannot prove and what the next pilot must earn before scale.</p></div><a className="button button-mint" href="/impact-dashboard">Open impact dashboard <ArrowRight aria-hidden="true" /></a></div><div className="committee-snapshot"><article><span>Reporting period</span><strong>{demonstrationReport.pilot.name}</strong><p>{demonstrationReport.pilot.period}</p></article><article><span>Second steps</span><strong>{demonstrationReport.metrics.appointmentsCompleted}</strong><p>Appointments completed from {demonstrationReport.metrics.appointmentsBooked} booked.</p></article><article><span>Warm referrals</span><strong>{demonstrationReport.metrics.referralsAccepted}</strong><p>Accepted by delivery partners from {demonstrationReport.metrics.referralsConsented} consented.</p></article><article><span>Scale position</span><strong>Conditional</strong><p>Test, measure and review before expansion.</p></article></div><section className="committee-review"><div className="committee-section-heading"><div><span className="eyebrow">Committee review</span><h2>Three questions before we scale.</h2></div><p>This is a decision aid, not a claim of proven impact. Each answer points back to the evidence and its limits.</p></div><div className="committee-question-grid">{committeeQuestions.map((question) => <article key={question.number}><div className="committee-question-top"><span>{question.number}</span>{question.icon}</div><h3>{question.title}</h3><p>{question.copy}</p><strong>{question.status}</strong></article>)}</div></section><section className="committee-actions"><div><span className="eyebrow">Next committee actions</span><h2>Protect the learning while the model is still small.</h2></div><ol><li><span>01</span><div><strong>Approve a measured next pilot</strong><p>Keep the public and online doors connected, with three- and six-month follow-up.</p></div><BadgeCheck aria-hidden="true" /></li><li><span>02</span><div><strong>Require a safeguarding check at each new location</strong><p>Confirm privacy, worker coverage, referral pathways and safe exit options before launch.</p></div><Scale aria-hidden="true" /></li><li><span>03</span><div><strong>Keep sponsor access aggregate-only</strong><p>Funding can support the bridge without purchasing participant-level visibility.</p></div><ShieldCheck aria-hidden="true" /></li></ol></section><section className="committee-boundary"><ShieldCheck aria-hidden="true" /><div><span className="eyebrow">The committee can see</span><h2>Enough to govern. Never enough to identify.</h2><p>Committee members see the impact dashboard, grouped outcomes, anonymised feedback and broad financial reporting. They do not see names, messages, referral records, worker notes or individual journeys.</p></div></section></main></div>;
}
...
export default function FirstStepApp() {
  ...
  const needsDemoState = ["start", "book", "resources", "demo", "account", "worker", "admin", "committee"].includes(parts[0] || "");
  let page: ReactNode;
  ...
  else if (parts[0] === "committee") page = <RoleGate required="committee" state={state}><CommitteePage /></RoleGate>;
  else page = <NotFound />;
